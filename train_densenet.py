...
def get_loss_function(loss_type, num_classes=10, ignore_index=-1):
    ...
    if loss_type == 'gce':
        ...
        return SCELoss(alpha=0.1, beta=1.0, num_classes=num_classes)  # Standard α=0.1, β=1.0 from paper
    elif loss_type == 'ce':
...
def main():
    ...
    if args.id == "CIFAR-10":
        ...
        selected_noise_name = "worst_label"  # Keep track of the name for logging
    ...
    class CIFAR10Noisy_GCE(Dataset):
        def __init__(self, trainset, noise_labels):
            self.dataset = trainset
            self.noise_labels = noise_labels
            assert len(self.dataset) == len(self.noise_labels), "Mismatch between dataset and noisy labels!"
    ...
    if in_dataset == "CIFAR-10":
        ...
        #trainset_NoiseDataset= CIFAR10Noisy_GCE(trainset, selected_noise_label)
    ...
    #criterion = get_loss_function('gce', num_classes=10, ignore_index=-1) 
...
def train(train_loader, model,loss_func_ce,  optimizer, epoch,num_classes,criterion,args,scheduler_f,optimizer_trans):
    ...
    model.train()
    #trans_matrices.train()
    
    end = time.time()
    total_samples = len(train_loader.dataset)  # Total samples in the dataset
    batch_size = args.bs
    M =args.M
    K=args.K
    #lambda_sparse = 0.001
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        #print(f"Shape of target_var (DenseNet output): {target_var.shape}")

        
        #optimizer_trans.zero_grad()

        # Compute DenseNet output
        final_output,reg_loss = model(input_var)  # Expected shape: [batch_size, M, K]
        
        # Compute Cross-Entropy Loss
        #ce_loss =  criterion(final_output, target_var,index)	
        ce_loss = loss_func_ce(final_output.log(), target_var.long())
...
def validate(val_loader, model,loss_func_ce, epoch,criterion,args):
    ...
    #lambda_sparse = 0.15
    #lambda_sparse =0
    end = time.time()  # Initialize the variable before the loop
    ...
    with torch.no_grad():
        for i, (input, target) in enumerate(val_loader):
           ...
           # Compute DenseNet output
            ...
            ce_loss = loss_func_ce(final_output.log(), target.long())
            #loss = ce_loss
            #ce_loss=0
            loss = ce_loss+lambda_sparse*reg_loss
            prec1 = accuracy(final_output.data, target, topk=(1,))[0]
            ...
            # measure elapsed time
            batch_time.update(time.time() - end)
            end = time.time()  # Reset the end variable
...
def save_checkpoint(args, state, is_best, filename):
    ...
    if args.model_type == 'densenet':
        directory = os.path.join("./checkpoints", f"{args.id}", "densenet_noodle_lamda_0.0005")
    elif args.model_type == 'resnet18':
