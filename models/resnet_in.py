...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, r, num_classes):
        self.r = r
        self.inplanes = 64
        self.block= block
        ...
        assert self.r !=0, Exception("Relevance ratio must be greater than 0")
        if self.r == 1:
            print("Starting Standard training without subspace learning")
            self.head = nn.Linear(512*block.expansion, num_classes)
        else:
            self.head = SNN(512*block.expansion, num_classes, r=self.r)
...
def resnet(r, num_classes, pretrained=True, depth=101, **kwargs):
    ...
    print(f"Running with ResNet-{depth} with relevance ratio = {r} and num_classes = {num_classes}")
    model = ResNet(block, num_blocks, r =r,num_classes = num_classes,**kwargs)
    if (pretrained):
